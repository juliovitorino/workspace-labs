package br.com.jcv.combination;
...
public class Combination {
    public static void main(String[] args) throws IOException {
        String[] palavras = {
                ...
                "junk", "justice", "justify", "keen", "keep", "ketchup", "key", "kick", "kid", "kidney",
                ...
                "suspect", "sustain", "swallow", "swamp", "swap", "swarm", "swear", "sweat", "sweet", "swift",
        ...
        final long[] c = {0};
        List<String> pairedStrings = IntStream.range(0, palavras.length)
                .boxed()
                .flatMap(i -> IntStream.range(0, palavras.length)
                        .filter(j -> i != j)
                        .mapToObj(j -> leftRightPad(c[0]++
                                + " = "
                                +palavras[i]
                                + " - "
                                +palavras[j],".", 35, "right")
        ...
        System.out.println("Total de combinações = " + pairedStrings.size());

        FileOutputStream fos = new FileOutputStream("/home/julio/Documents/pares.txt");
        pairedStrings.forEach(i -> {
            try {
    ...
    private static String leftRightPad(String applyStr, String character, int length, String type) {
        if(applyStr.isEmpty()) return applyStr;
        int applyStrLenght = applyStr.length();
        if( applyStrLenght >= length) return applyStr;

        int characterfill = length - applyStrLenght + 1;
        StringBuilder sbLeftRightPad = new StringBuilder();
        sbLeftRightPad.append(String.valueOf(character).repeat(characterfill - 1));
        return type.equals("left")
                ? sbLeftRightPad + applyStr
                : applyStr + sbLeftRightPad;
    }
